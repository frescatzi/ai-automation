...
Rules:
- FIRST read `AGENTS.md` at the repo root and follow its wiki conventions and workflows exactly.
...
2. Read `AGENTS.md`, `index.md`, and the tail of `log.md` to load the conventions and see what
...
3. For each new/changed source: extract the durable facts and fold them into the right `wiki/`
   ...
   home exists. Follow the naming, structure, and [[wikilink]] conventions from AGENTS.md.
