---
type: raw
title: "SOP_Lumina_intake-et-publish"
source_url: "drive:1oAv-jePLTPJbsgSvdBcJer_qW2IMkmdI"
captured: 2026-06-24
vault: ai-automation
brand: null
immutable: true
...
---
type: sop
title: "SOP Lumina — Robot d'intake (Drive→GitHub) & Publication Notion idempotente"
status: active
vault: ai-automation
updated: 2026-06-24
---
...
> Procédure **propre à l'infrastructure Lumina (AFTRSN)**. Pour la version réutilisable/abstraite, voir `SOP_generique_pipeline-source-vers-vues.md`.
> Couvre 2 workflows n8n construits/réparés le 2026-06-24 : le **robot d'intake v2** et le **publish Notion idempotent**.
...
## 5. Challenges rencontrés & solutions (cette session)

| # | Challenge | Cause | Solution |
|---|---|---|---|
| 1 | Doublons Notion (~40→74 entrées) | Publish non idempotent (re-création) | **Upsert par fichier** : archive ancienne + recrée |
| 2 | Filtre Notion renvoie 0 résultat | `{{ }}` dans body JSON envoyé en texte brut | Body en **mode Expression** + `JSON.stringify` |
| 3 | Pas de match sur l'URL | `sourceUrl` (Code Parsing) tronqué `…/blob/main/` | Matcher sur `payload.properties.Source.url` (Build payload) |
| 4 | Page Notion fantôme (titre vide) | Fil parasite `Query Notion DB → Code (Parsing)` injecte 1 item non-wiki | Code (Parsing) reçoit **uniquement** Set Document |
| 5 | README publié | Filter trop large | Condition `path does not contain README` |
| 6 | Perte de 74 entrées en test | Branche archive lancée seule | Ne jamais single-step l'archive ; Git = source → re-remplir = 1 run publish |
| 7 | Classifieur en panne (503) | Gemini free tier surchargé (même avec retry) | Bascule classifieur sur **OpenAI** (`gpt-4o-mini`) ; adapter parsing `choices[0].message.content` |
| 8 | 1 seul fichier traité | « Fetch Test Event » = 1 item | Node **Search files and folders** (liste tout l'inbox) |
| 9 | Code_Parse ne sort qu'1 item | `Run Once for All Items` + `.first()`/`.all()[0]` | **Run Once for Each Item** + `$json` + `$('…').item` |
| 10 | Delete « Referenced node doesn't exist » | mauvais nom de node référencé | `{{ $('Code_Parse').item.json.fileId }}` |
| 11 | GitHub 422 « sha wasn't supplied » | Fichier déjà existant (backlog) | `On Error = Continue` le temps d'un drain, **puis remettre Stop** |
| 12 | Fichiers non supprimés de Drive | Delete jamais atteint (Gemini plantait) en amont | Réparer l'amont (OpenAI) ; Delete après Create réussi |
...
- **Idempotence** : intake = traité→supprimé (pas de re-collision) ; publish = upsert par fichier ; pgvector = TRUNCATE+reload.
- **Tags n8n** : Statut/Déclencheur/Domaine (voir `lumina-systeme-reference.md` §8). Workflows : `🟢 prod` + `⏰ scheduled` + `📥 intake` (robot) / `🧠 memory` (publish).
- **Jamais** de PAT/token dans le chat ; jamais d'édition manuelle de pgvector/Notion (dérivés).
