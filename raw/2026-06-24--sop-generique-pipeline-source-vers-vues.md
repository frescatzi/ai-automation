---
type: raw
title: "SOP_generique_pipeline-source-vers-vues"
source_url: "drive:1eybdKsVF1Ty7WbGwY7kTkwKBVi9qDc5z"
captured: 2026-06-24
vault: ai-automation
brand: null
immutable: true
...
> Procédure **générique et réutilisable** (toute marque / tout projet). Remplace les `<placeholders>`. Version concrète Lumina : `SOP_Lumina_intake-et-publish.md`.
...
- **Une source de vérité unique** (ex. un repo Git). Tout le reste (index vectoriel, base lisible) = **sortie dérivée, régénérable, jamais éditée à la main**.
...
|---|---|---|---|
| **Expressions** | Filtre/áppel renvoie 0 / valeur littérale | `{{ }}` dans un body JSON non évalué | Body en **mode Expression** + `JSON.stringify(objet)` |
| **Identité/clé** | L'upsert ne matche jamais | Clé tronquée/différente de la valeur écrite | Filtrer sur **la même donnée** que celle stockée (souvent après le node « payload ») |
| **Items parasites** | Entrée « fantôme » (titre/clé vide) | Un node Code reçoit **2 flux** en entrée | Garder **une seule entrée** par node Code ; séparer les flux |
| **Itération** | 1 seul item traité sur N | Code en `Run Once for All Items` avec `.first()` | Passer en **Run Once for Each Item** + `$json` + `$('…').item` |
| **Références** | « Referenced node doesn't exist » | Mauvais nom de node | Référencer le **nom exact** ; préférer un champ porté par le dernier node fiable |
| **Backlog** | Seul 1 fichier traité, le reste reste | Trigger « 1 événement » au lieu d'une **liste** | Node **list/search** qui énumère tout l'inbox |
| **Conflit d'écriture** | 4xx type « already exists / sha manquant » | Cible déjà présente | Upsert (update avec clé) **ou** tolérer l'erreur le temps d'un drain ponctuel, puis re-durcir |
| **Fournisseur LLM** | 5xx « high demand » persistant | Palier gratuit surchargé | **Basculer de fournisseur/modèle** (ex. OpenAI) + **Retry On Fail** |
| **Suppression dangereuse** | Données perdues | Étape destructive jouée seule / avant confirmation | Supprimer **après** écriture confirmée (`On Error = Stop`) ; jamais single-step la branche destructive ; garder la source régénérable |
| **Données figées** | Tests trompeurs | Sortie de node **épinglée**/cache | Dé-épingler, re-fetch ; en prod, pas de pin |
